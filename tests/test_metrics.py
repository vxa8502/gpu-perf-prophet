"""Tests for src/models/metrics.py (sMAPE, median APE): hand-computed expected values (not sklearn or the functions' own internals) plus zero-denominator floor edge cases."""
...
class TestSmape:
    ...
    def test_multi_row_hand_computed(self):
        # Hand-computed: row1 2*10/210=0.0952380952, row2 2*20/380=0.1052631579, mean=0.1002506265.
    ...
    def test_symmetric_in_over_and_under_prediction(self):
        # sMAPE's whole point vs. plain MAPE: overshoot and undershoot by the same absolute amount around the same true value score identically.
        over = smape([100], [150])
        under = smape([150], [100])
        assert math.isclose(over, under)

    def test_zero_true_and_zero_pred_does_not_raise_and_is_zero(self):
        # Denominator floored at machine epsilon; numerator is also 0, so the ratio is 0, not NaN/inf.
        assert smape([0], [0]) == 0.0

    def test_bounded_in_zero_to_two(self):
        # sMAPE's fractional form is bounded in [0, 2] by construction (numerator <= denominator always, times 2).
...
class TestMedianApe:
    ...
    def test_zero_true_denominator_floored_not_raising(self):
        # Pins the exact expected value (division by machine epsilon specifically), not just "finite and positive" — a looser floor (e.g. 1.0) would still be finite/positive but silently wrong by ~16 orders of magnitude, confirmed by mutation: swapping the floor to 1.0 left the old finite-and-positive assertion passing; uses sys.float_info.epsilon (not metrics._EPS) so a typo'd _EPS binding in the module under test can't also poison this expectation.
